...
export function LeadershipReviewsPage() {
  ...
  const { data: usersData } = useQuery({ queryKey: ['leadership-users'], queryFn: () => api.get<{ users: Person[] }>('leadership/users'), enabled: isSystemAdmin });
  const { data: reviewsData, isLoading: reviewsLoading } = useQuery({ queryKey: ['leadership-reviews'], queryFn: () => api.get<{ reviews: ReviewListItem[] }>('leadership/reviews'), enabled: isSystemAdmin });
  const { data: meetingsData, isLoading: meetingsLoading } = useQuery({ queryKey: ['leadership-one-on-ones'], queryFn: () => api.get<{ meetings: Meeting[] }>('leadership/one-on-ones'), enabled: isSystemAdmin });
  ...
  return <AppLayout title="Leadership Reviews" actions={<div style={{ display: 'flex', gap: 8 }}>
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      {activeReviewId ? <ReviewEditor reviewId={activeReviewId} onBack={() => setActiveReviewId(null)} /> : tab === 'reviews' ? <>
        ...
        <div className="card">
          {reviewsLoading && <div style={{ padding: 32 }}><Spinner /></div>}
          {!reviewsLoading && reviews.length === 0 && <EmptyState icon="◆" title="No monthly reviews" description="Create a review and select your direct reports." action={<Button variant="primary" onClick={() => setShowReview(true)}>Create first review</Button>} />}
        ...
        {meetingsLoading && <div style={{ padding: 32 }}><Spinner /></div>}
        {!meetingsLoading && meetings.length === 0 && <EmptyState icon="◉" title="No one-on-ones recorded" description="Record the first meeting with a direct report." />}
    ...
    <CreateReviewModal open={showReview} users={usersData?.users ?? []} error={error} setError={setError} onClose={() => setShowReview(false)} onCreated={id => { qc.invalidateQueries({ queryKey: ['leadership-reviews'] }); setShowReview(false); setActiveReviewId(id); }} />
    <CreateMeetingModal open={showMeeting} users={usersData?.users ?? []} error={error} setError={setError} onClose={() => setShowMeeting(false)} onCreated={() => { qc.invalidateQueries({ queryKey: ['leadership-one-on-ones'] }); setShowMeeting(false); setTab('one-on-ones'); }} />
  </AppLayout>;
}

function CreateReviewModal({ open, users, error, setError, onClose, onCreated }: { open: boolean; users: Person[]; error: string; setError: (value: string) => void; onClose: () => void; onCreated: (id: string) => void }) {
...
function CreateMeetingModal({ open, users, error, setError, onClose, onCreated }: { open: boolean; users: Person[]; error: string; setError: (value: string) => void; onClose: () => void; onCreated: () => void }) {
  ...
  return <Modal open={open} onClose={onClose} title="Record one-on-one" footer={<><Button variant="secondary" onClick={onClose}>Cancel</Button><Button variant="primary" loading={create.isPending} onClick={() => { if (!reportId || !meetingDate) { setError('Direct report and meeting date are required.'); return; } create.mutate(); }}>Save meeting</Button></>}>
    {error && <div style={{ marginBottom: 12 }}><Alert type="error">{error}</Alert></div>}
    <Select label="Direct report" value={reportId} onChange={event => setReportId(event.target.value)} options={[{ value: '', label: 'Select a user' }, ...users.map(user => ({ value: user.id, label: `${user.name} (${user.email})` }))]} />
