...
function EntryEditor({ reviewId, entry, onSaved }: { reviewId: string; entry: ReviewEntry; onSaved: () => void }) {
  ...
  const aiDraft = useMutation({
    ...
    onSuccess: ({ draft }) => {
      setAchieved(text(draft.tasksAchieved)); setProgress(text(draft.inProgress)); setPlanned(text(draft.planned));
      setOneOnOne(text(draft.oneOnOneSummary)); setLearning(text(draft.learningDevelopment)); setFeedback(text(draft.managerFeedback));
      setSaved(false); setAiMessage({ type: 'success', text: 'AI draft loaded from this reporting period. Review and edit it, then click Save direct-report slide.' });
    },
